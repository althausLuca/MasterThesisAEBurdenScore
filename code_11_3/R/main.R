...
scenario$treatment$gap_time <- scenario$control$gap_time*0.75
## Single group scores
...
treatment.gamma.1.5 <-  simulate_group(scenario$treatment,susceptibility_parameter = list("gamma", 1.5))
...
#simulate  trials
#profvis({
  simulate_trials_from_scenario(scenario ,susceptibility_parameter = list("gammma",0.5), n_sim = n_sim, death = FALSE, save= TRUE)
#})

source("R/methods/run_methods.R")

# run models
run_methods_from_file(scenario$name, n_it = -1)
